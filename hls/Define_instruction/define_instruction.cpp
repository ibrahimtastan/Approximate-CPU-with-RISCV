...
int main()
{
    ...
    std::string filepath_filepath ="filepath.txt" ;
...
*/
    ...
    uint32_t mem_start_adress = 0xa000  ;
    uint8_t mem;
	i =0 ;
    int    memory_size =  80*1024 + 10*1024;
   ...
   int prev_size =  mnemonic_vector.size();
    ...
    fprintf(myfile, "const uint32_t inst_mem[ %d ] = { ",instruction_num);
   ...
   fprintf(myfile, "static uint8_t mem[ %d ] = { ",memory_size);
